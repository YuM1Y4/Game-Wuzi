...
class GoBang(QWidget):
    ...
    def initUI(self):
        ...
        self.pieces = [LaBel(self) for i in range(225)]  # 新建棋子标签，准备在棋盘上绘制棋子，共15*15
        self.positions=[[] for i in range(225)]
        self.states = [EMPTY for i in range(255)]
    ...
    def coordinate_transform_pixel2map(self, x, y):
        ...
        i, j = int(round((x - MARGIN) / GRID)), int(round((y - MARGIN) / GRID))
    ...
    def keyPressEvent(self, event):
        ...
        if (event.key() == Qt.Key_O) and QApplication.keyboardModifiers() == Qt.ShiftModifier:
           ...
           '''
        #玩家下棋时，才允许悔棋
        if (event.key() == Qt.Key_Left) and self.ai_down == True:
            self.regret()
        if (event.key() == Qt.Key_Right) and self.ai_down == True:
            self.Recovery()
    ...
    def regret(self):
        ...
        if(self.step > 0):
            Maxstep = self.step + 1
            self.step -= 1
            self.pieces[self.step].setGeometry(-100, -100, PIECE, PIECE)  # 删除棋子
            i,j=self.positions[self.step]
            r_i, r_j = self.coordinate_transform_map2pixel(i, j)
        ...
        if(self.step > 0):
            ...
            R_i,R_j = self.coordinate_transform_map2pixel(i,j)
            self.chessboard.draw_xy(i, j, EMPTY)
            #x, y = self.coordinate_transform_pixel2map(i, j)
        self.piece_now = BLACK
        #更新AI的指示线
        if(self.step >0):
            i,j=self.positions[self.step-1]
            self.x, self.y = self.coordinate_transform_map2pixel(i, j)
        else:
    ...
    def Recovery(self):
        if (self.step < Maxstep):
            self.step
            self.pieces[self.step].setGeometry(r_i,r_j,PIECE,PIECE)
            i,j=self.positions[self.step]
            self.chessboard.draw_xy(i,j,self.states[Maxstep])
        if (self.step < Maxstep):
            self.step
            self.pieces[self.step].setGeometry(R_i,R_j,PIECE,PIECE)
            i,j=self.positions[self.step]
